...
internal static class MemberPolyfills_Net70_IntPtr
{
    extension(IntPtr)
    {
        ...
        public static bool TryParse(string s, IFormatProvider? provider, out IntPtr result)
        {
            if (IntPtr.Size == 4)
            {
                var success = int.TryParse(s, NumberStyles.Integer, provider, out var intResult);
                result = new IntPtr(intResult);
                return success;
            }
            else
            {
                var success = long.TryParse(s, NumberStyles.Integer, provider, out var longResult);
